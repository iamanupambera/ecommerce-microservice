...
export class GatewayCacheService {
  ...
  async saveUserSelectedCategory(key: string, value: string) {
    try {
      await this.redisService.redis.set(key, value);
    } catch (error) {
      this.logger.log(
        'error',
        `${GatewayCacheService.name} saveUserSelectedCategory() method error:`,
        error,
      );
    }
  }
